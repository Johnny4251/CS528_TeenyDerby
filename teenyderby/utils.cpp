                 ...
                 DerbyState* derby_state)
{
    ...
    for (size_t i = 0; i < bin_files.size(); ++i) {
        derby_state[i].id            = (int)i;
        derby_state[i].sensor_target = 0;
        derby_state[i].speed         = 0;
        derby_state[i].health        = 100;    // temporary; overwritten by randomize_cars
        derby_state[i].isDead        = false;  // explicitly alive
    }
...
void drawHealthBar(Tigr* win, const Car& car)
{
    ...
    // Use per-car max health if set
    int maxHealth = (car.maxHealth > 0) ? car.maxHealth : 100;
